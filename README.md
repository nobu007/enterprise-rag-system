...
- **🔒 Enterprise-Ready**
  ...
  - **Document Relationship Graph** (planned; relationship endpoints are not available yet)
...
The document relationship graph is planned but is not currently implemented.
The mounted `/api/v1/relationships` router is a placeholder and exposes no
relationship endpoints yet. See the roadmap for its current status.
