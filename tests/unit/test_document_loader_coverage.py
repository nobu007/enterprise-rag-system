...
Targets previously-uncovered branches: DocumentLoader.load_pdf
(ImportError when pypdf absent + happy path via fake pypdf), the
load_directory .pdf branch + per-file error handler, and the two
TextSplitter.split_text branches (oversized first part; fixed-size
fallback when no configured separator matches).
"""
import sys
...
class TestLoadPdf:
    ...
    def test_load_markdown_returns_document(self, tmp_path):
        target = tmp_path / "note.md"
        target.write_text("# Title\nbody", encoding="utf-8")

        doc = DocumentLoader.load_markdown(str(target))
